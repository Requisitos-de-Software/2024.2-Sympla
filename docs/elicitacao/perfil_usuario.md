## Introdução

No objetivo de entender melhor as necessidades e comportamentos dos usuários de um sistema, é fundamental desenvolver um perfil de usuário. Esse perfil servirá como uma representação das características dos indivíduos que irão utilizar a plataforma, com o intuito de garantir que suas expectativas e objetivos sejam atendidos de forma eficaz.

## Metodologia

Para a criação dos perfis de usuário, foi utilizado os dados adquiridos pela técnica do [Questionário](https://requisitos-de-software.github.io/2024.2-Sympla/elicitacao/tecnicas/questionario/). O questionário possuía 8 questões sobre aspectos gerais do usuário do aplicativo, onde foi divulgado em grupos atráves dos aplicativos whatsapp e telegram entre o período de 13 a 18 de Novembro de 2024. Informações mais detalhadas quanto ao questionário, estão disponíveis na página do artefato.

## Resultados

Os resultados no questionário teve um total de 54 respostas e, todas de acordo com o termo de consetimento exibido no início do questionário. O resultado das respostas são apresentados nas figuras de 1 a 8.

De acordo com o gráfico apresentado na figura 1, temos 82,2% representando a faixa etária da maioria dos participantes , que estão de 18 a 24 anos.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 1</b> - Pergunta 1.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1105751934&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com a figura 2, o gênero dos participantes em sua maioria é feminino representando 60% das respostas e o gênero masculino representando os outros 40%.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 2</b> - Pergunta 2.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1209515326&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 3, o grau de escolaridade dos participantes apresenta taxas como 64,4% no Ensino Superior Incompleto, depois o Ensino Superior Completo com 20%, o Ensino Médio completo representaram com 11,1%, pós graduação e ensino fundamental completo com 2,2%.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 3</b> - Pergunta 3.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1891491603&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 4 sobre o nível de intimidade com a tecnologia, temos o maior número no nível 5 de intimidade, chegando a 44,4%.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 4</b> - Pergunta 4.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1530431102&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 5 sobre encontrar alguma dificuldade em sites, temos que a maioria acaba buscando em todas as opções no site até achar o que procura, chegando aos 86,7%.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 5</b> - Pergunta 5.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1389444095&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 6 em relação a frequência que participam de eventos, temos 40% de pessoas que vão a eventos frequentemente, 37,8% que vão as vezes e 22,2% que vão raramente.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 6</b> - Pergunta 6.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=289153411&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 7 sobre a frequência de compras online, temos que 37,8% compram coisas online frequentemente, 31,1% compram as vezes, 24,4% compram sempre e 6,7% compram raramente.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 7</b> - Pergunta 7.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=1980204085&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

De acordo com o gráfico apresentado na figura 8 sobre a frequência de compras de ingressos online, temos que 35,6% optam por comprar online as vezes, 26,7% optam por comprar sempre, 24,4% optam por comprar frequentemente e 13,3% raramente compram online.

<figure markdown>
<font size="4"><p style="text-align: center"><b>Figura 8</b> - Pergunta 8.</p></font>
<iframe style="border:3px solid blue" width="648" height="401" seamless frameborder="0" scrolling="no" width="600" height="371" seamless frameborder="0" scrolling="no" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQBKvPNRGAkOBk2oHFzEnW5kqLTSQ8JKvrQdmrmp8rF0POqylGDGWV-VQjFjGt9dhb6reUp-NRKlkqR/pubchart?oid=623768043&format=interactive"></iframe>
<font size="3"><p style="text-align: center"><b>Fonte:</b> Elaborado por <a href="https://github.com/VHbernardes">Victor Hugo</a>, 2024</p></font>
</figure>

## Definição do Perfil de Usuário

Após analisar as respostas coletadas, foi possível traçar o perfil de usuário para o aplicativo, levando em consideração as características demográficas, comportamentais e de interação com a tecnologia dos participantes.

- **Gênero**:
    Predominantemente feminino. A maioria dos participantes que responderam ao questionário se identificam como femininas (60%), com uma participação significativa também do gênero masculino (40%).
- **Idade**:
    Entre 18 e 24 anos. A faixa etária dominante dos participantes é composta por jovens adultos, com 82,2% dos respondentes na faixa etária de 18 a 24 anos, indicando um público mais jovem e possivelmente universitário ou em início de carreira profissional.
- **Experiência**:
    Experientes com tecnologia. Com um alto índice de intimidade com a tecnologia, onde 44,4% dos participantes se consideram altamente familiarizados com dispositivos digitais.
- **Atitudes**:
    Tecnófilos. Os participantes demonstram uma grande afinidade com a tecnologia, com destaque para o alto nível de intimidade com a tecnologia (nível 5) e o fato de serem ativos na utilização de plataformas online. Isso se reflete no comportamento de frequentes compras online, bem como na frequência de participação em eventos e compra de ingressos online.
- **Tarefas Primárias**:
    Compra e consulta. Os usuários frequentemente realizam compras online, com 37,8% comprando com frequência e 31,1% comprando de vez em quando. Além disso, a maioria consulta e explora diversas opções dentro de sites até encontrar o que procuram. Isso aponta para uma preferência por comprar e consultar informações online.

## **Bibliografia**

> Barbosa, S. D. J.; Silva, B. S. da; Silveira, M. S.; Gasparini, I.; Darin, T.; Barbosa, G. D. J. (2021) Interação Humano-Computador e Experiência do usuário. Autopublicação. ISBN: 978-65-00-19677-1. Disponível em: [https://aprender3.unb.br/pluginfile.php/2972437/mod_resource/content/2/ihc-ux-%20Personas.pdf](https://aprender3.unb.br/pluginfile.php/2972437/mod_resource/content/2/ihc-ux-%20Personas.pdf). Acesso em: 18 de Nov. 2024


## Histórico de Versões

<center>


| Versão |          Descrição              |     Autor      |      Data      |   Revisor     |
|:------:|:-------------------------------:|:--------------:|:--------------:|:-------------:|
|  1.0   | Criação do documento |  [Victor Hugo](https://github.com/VHbernardes) | 12/11/2024   | [Gabriel Scheidt](https://github.com/Gxaite) |
|  1.1   | Adição dos resultados e criação do perfil do usuário |  [Victor Hugo](https://github.com/VHbernardes) | 18/11/2024   | [Rafael Pereira](https://github.com/rafgpereira) |


</center>
